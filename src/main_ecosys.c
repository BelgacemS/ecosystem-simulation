...
int main(void) {
  ...
  FILE *file = fopen("data/population_stats.txt", "w"); // Ouverture du fichier pour enregistrer les statistiques de population
  ...
  clear_screen();
  afficher_ecosys(gazelle, lion, tab_herbe); // Affichage de l'état initial de notre écosystème
  ...
  while(compte_animal_it(gazelle)>0 && nbIt<nbMaxIt){
    ...
    fprintf(file,"%d %d %d\n", nbIt, compte_animal_it(gazelle), compte_animal_it(lion)); //écriture dans le fichier pour chaque itération du numéro de l'itération actuelle, du nombre de proies et de prédateurs restants
    ...
    clear_screen();
    afficher_ecosys(gazelle, lion, tab_herbe);
  ...
  fprintf(file,"%d %d %d\n", nbIt, compte_animal_it(gazelle), compte_animal_it(lion)); //écriture pour le dernier tour dans le fichier
